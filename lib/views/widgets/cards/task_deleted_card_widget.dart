...
class TaskDeletedCardWidget extends StatelessWidget {
  final TaskModel task;

  const TaskDeletedCardWidget({
    super.key,
    required this.task,
  });

  void _restaureData({required BuildContext context}) async {
    if (await MessagesLibrary.showDialogAlert(context: context, child: const Text("Voulez-vous restaurer cette tâche?")) == true) {
      if (!context.mounted) return;
      EasyLoading.show(status: "Restauration de la tâche ...");
      bool result = await context.read<TasksProvider>().manageOneTrashTask(map: task);
      if (!context.mounted) return;
      EasyLoading.dismiss();
      if (result == false) {
        MessagesLibrary.showSnackBar(context: context, message: "Erreur lors de la restauration de la tâche!", typeMessage: TypeMessage.typeError);
      } else {
        MessagesLibrary.showSnackBar(context: context, message: "La tâche est restaurée!");
      }
    }
  }
    ...
    return Card(
      ...
      child: ListTile(
        ...
        trailing: IconButton(
          ...
          icon: const FaIcon(FontAwesomeIcons.trashCanArrowUp, color: ThemesLibrary.kColorDefaultRed),
          onPressed: () => _restaureData(context: context),
        ),
      ),
    );
  }
